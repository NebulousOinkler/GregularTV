...
- **When there's a break:** after an episode, a slot is always rounded up to the `padTo` boundary, so episodes keep the channel on the half hour with commercials after each. After a film (anything not an episode), only when that leaves 10 minutes or less (`longestBreak`); otherwise the next programme starts as soon as the film ends, off the boundary. So a film ending at 1:32 is followed by the next programme at 1:32, not 28 minutes of commercials; if that's an episode, its break brings the channel back to the half hour.
...
- **All default channels** use `"filler": "derangement"` and `"padTo": 30`, so every episode is followed by a break up to the next half hour (a 22-minute episode gets 8 minutes, a 44-minute one 16), and a film only when that's 10 minutes or less (a 100-minute film is followed straight away by the next programme).
