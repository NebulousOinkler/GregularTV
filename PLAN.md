...
- **Player and UI:** clips play as ordinary airings with seamless hand-off. The banner shows "Commercial break · Back at h:mm with <next programme>" and stays up for the whole break.
...
- **All default channels** use `"filler": "derangement"` and `"padTo": 30`, so every programme is followed by a break up to the next half hour (a 22-minute episode gets 8 minutes; a 100-minute film, 20).
...
- **Lightest load on the server:** commercials never make Jellyfin re-encode video, and ignore the quality setting's speed test. They ask once for the original file within the current cap (Auto's last measurement); played as-is or only remuxed, it costs the server almost nothing. If the video would have to be re-encoded (`PlaybackSource.reencodesVideo`), the clip is skipped: its time is blank, with the banner, and the clip is remembered in memory for the session (cleared when the quality changes), so it isn't asked about again. Clips that are MP4 H.264/AAC at a modest bitrate always play.
